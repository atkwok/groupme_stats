...
with open(".groupme.env") as f:
    a = bytes(f.readline().strip(), "ascii")
...
def fetch_all_messages(group_id):
    ...
    with open("all_messages_for_" + str(group_id) + ".txt", "w") as f:
...
def load_messages(group_id, load_recent=_RELOAD):
    all_messages = []
    with open("all_messages_for_" + str(group_id) + ".txt") as f:
        for line in f:
...
def load_all_messages(load_recent=_RELOAD):
    if load_recent:
        ...
        with open("all_messages_for_all.txt", "w") as f:
            f.write(json.dumps(all_messages))
        return all_messages
    else:
        with open("all_messages_for_all.txt") as f:
            all_messages = []
            for line in f:
                all_messages += json.loads(line)
        return all_messages
...
#################################################################
def aggregate_msg_by_user(group_id):
...
def count_msg_by_user(group_id):
    users = aggregate_msg_by_user(group_id)
    return {i: len(users[i]) for i in users}

def likes_per_user(group_id):
    users = aggregate_msg_by_user(group_id)
    return {user: sum([len(msg["favorited_by"]) for msg in users[user]]) for user in users}

def likes_per_message_per_user(group_id):
    users = aggregate_msg_by_user(group_id)
